...
Overlay. prototype =
{
	...
	get overlayDocument ()
	{
		dLOG ("overlayDocument: " + this. path + ": " + this. fileName)
		if (!this. _overlayDocument)
		{
			...
			var s = sis. read (instr. available ());
			//this. _overlayDocument = dp. parseFromStream (instr, "UTF-8", instr. available (), "text/xml");
			this. _overlayDocument = dp. parseFromString (s, "text/xml");
			dLOG ("_overlayDocument: " + this. _overlayDocument. documentElement. textContent);
		}
...
AppObject. prototype =
{
	...
	getButton: function (sButtonId)
	{
		...
		dLOG ("palId: " + this. paletteId + ": " + this. windowId);
