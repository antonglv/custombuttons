...
cbCustomButtonsService. prototype =
	...
	{
	    clonedButton = clonedButton. QI (nsIDOMNode);
	...
	removeRDFResource: function (ds, res)
	{
	    ...
	    {
		ca = alo. getNext ();
		if (ca instanceof CI. nsIRDFResource)
		{
		    ta = ds. GetTarget (res, ca, true);
		    if (ta instanceof CI. nsIRDFResource)
			this. removeRDFResource (ds, ta);
		    else if (ta instanceof CI. nsIRDFLiteral)
	...
	unPersist: function (resId)
	{
		...
		{
		    cta = targets. getNext ();
		    if (cta instanceof CI. nsIRDFResource)
		    {
	...
	{
	    removedButton = removedButton. QI (nsIDOMNode);
